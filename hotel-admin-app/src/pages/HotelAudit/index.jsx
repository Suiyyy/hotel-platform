...
import { Table, Button, Tag, Modal, Input, message, Space, Card, Popconfirm, Image, Tabs, notification, Typography } from 'antd'
import { CheckOutlined, CloseOutlined, PoweroffOutlined, SearchOutlined, BellOutlined } from '@ant-design/icons'
import { useHotelStore } from '../../store/hotelContext'
import { getBaseUrl } from '../../services/hotelApi'
import './index.css'
...
const HotelAuditPage = () => {
  ...
  const columns = [
    ...
    { title: '星级', dataIndex: 'star', width: 80, render: (star) => `${star}星级` },
    { title: '价格', dataIndex: 'price', width: 100, render: (price) => `¥${price}` },
    { title: '审核状态', dataIndex: 'status', width: 100, render: (status) => getStatusTag(status) },
    {
      title: '上线状态',
      dataIndex: 'isOnline',
      width: 100,
  ...
  return (
    <div className="hotel-audit-container">
      <Typography.Title level={4} className="page-title">酒店审核管理</Typography.Title>
      <Card>
        <Tabs activeKey={statusFilter} onChange={(k) => setStatusFilter(k)} items={tabItems} />
