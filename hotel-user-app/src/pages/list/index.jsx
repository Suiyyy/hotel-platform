...
const ListPage = () => {
  ...
  useEffect(() => {
    ...
    if (params.ai === '1') {
      setIsAiMode(true);
      const savedQuery = Taro.getStorageSync('aiSearchQuery');
      if (savedQuery) setAiQuery(savedQuery);
      try {
        const results = JSON.parse(Taro.getStorageSync('aiSearchResults') || '[]');
        setDisplayedHotels(results);
        setHasMore(false);
      } catch {
        setDisplayedHotels([]);
      }
      // 清除 storage 中的 AI 搜索数据
      Taro.removeStorageSync('aiSearchResults');
      Taro.removeStorageSync('aiSearchQuery');
      return;
    }
  }, []);
  ...
  return (
    <View className="list-page">
      ...
      {isAiMode && (
        <View className="ai-result-banner">
          <Text className="ai-result-icon">&#x2728;</Text>
          <Text className="ai-result-text">AI 搜索: {aiQuery}</Text>
          <View
            className="ai-result-close"
            onClick={() => { setIsAiMode(false); loadHotels(true); }}
            onTap={() => { setIsAiMode(false); loadHotels(true); }}
          >
            <Text className="ai-result-close-text">清除</Text>
          </View>
        </View>
      )}
